<?php
	...
	if(!empty($_GET['qClass'])) {
		$mArr = array();
		$q1 = "SELECT * FROM item_tree WHERE parent = ".$_GET['qClass'];
	...
	if(!empty($_GET['qType'])){
		$tArr = array();
		$q2 = "SELECT * FROM itemList WHERE parent = ".$_GET['qType'];
	...
	if(!empty($_GET['qName'])){
		...
		$q2 = "SELECT * FROM `modifiers` WHERE `parent` LIKE '%".$item['parent']."%'".$vars;
	...
	if(!empty($_GET['gClass'])) {
		$mArr = array();
		$q1 = "SELECT * FROM gja_tree WHERE parent = ".$_GET['gClass'];
		$result = $GLOBALS['mysqli']->query($q1);
		while($row = $result->fetch_assoc()){
			$m2 = array($row['id'],$row['name']);
			array_push($mArr,$m2);
	...
	if(!empty($_GET['gType'])){
		$tArr = array();
		$q2 = "SELECT * FROM gjaList WHERE parent LIKE '%".$_GET['gType']."%'";
		$result = $GLOBALS['mysqli']->query($q2);
		while($row = $result->fetch_assoc()){
			$title = $row['itemName']." (".$row['itemLVal']."-".$row['itemHVal'].")";
